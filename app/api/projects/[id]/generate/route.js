...
async function verifyContractorLinks(markdown) {
...
async function generateContractors(openai, model, context, projectId, revision = "") {
  ...
  // Validate every non-directory link before saving.
  const verified = await verifyContractorLinks(content);
  content = verified.markdown;
