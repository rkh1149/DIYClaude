...
export function projectContext(project, attachments = []) {
...
CLARIFICATION Q&A:
${answers}${attachmentContext(attachments)}`;
}

function attachmentContext(attachments) {
  const withSummary = (attachments || []).filter((a) => a.summary?.trim());
  if (!withSummary.length) return "";
  let out = `\n\nATTACHMENT DETAILS — the homeowner attached these to the project. Treat the details below as PRIORITY information that overrides any conflicting assumptions:`;
  for (const a of withSummary) {
    const label = a.kind === "image" ? "PHOTO OF THE ACTUAL SPACE/PROJECT" : `DOCUMENT (${a.filename})`;
    const verified = a.user_verified ? " [HOMEOWNER-VERIFIED — authoritative]" : "";
    out += `\n\n${label}${verified}:\n${a.summary.trim()}`;
  }
  return out;
}
...
export const TYPE_PROMPTS = {
  ...
  contractors: `You have web search. Find REAL, currently operating contractors near the homeowner's ZIP/postal code for each trade this project needs.

STRICT RULES — follow exactly:
1. Include each business's website as a markdown link whenever any search result shows one — most established contractors have a site, so actively look for it (search the business name + town if needed). Copy the URL exactly as it appears in search results. NEVER construct, guess, or recall a URL from memory.
2. Only list a business under a trade if your search results confirm it actually performs that trade. Prefer 2-3 confirmed businesses per trade over a longer uncertain list.
3. Only when repeated searching shows no website at all, list the business with name, town, and phone only. This should be the exception, not the rule.
4. After the businesses for each trade, add a "Find more:" line with these always-valid search links, substituting the trade and the homeowner's ZIP/postal code (URL-encode spaces as +): [Google Maps](https://www.google.com/maps/search/TRADE+near+CODE) and [Yelp](https://www.yelp.com/search?find_desc=TRADE&find_loc=CODE). Also mention Angi.com (US) or HomeStars.com (Canada) by name.

Note license/insurance verification steps for the homeowner's state or province. End with: questions to ask when getting quotes, and a reminder that the homeowner must check recent reviews and confirm each business is still operating before contacting them.`,
