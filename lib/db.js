...
async function createTables() {
  ...
  // Migration for databases created before multi-attachment support.
  await sql`ALTER TABLE attachments DROP CONSTRAINT IF EXISTS attachments_project_id_kind_key`;
}

export async function getAttachments(projectId, { includeData = false } = {}) {
  const { rows } = includeData
    ? await sql`SELECT id, kind, filename, mime, data, summary, user_verified
                FROM attachments WHERE project_id = ${projectId} ORDER BY id`
    : await sql`SELECT id, kind, filename, mime,
                CASE WHEN kind = 'image' THEN data ELSE NULL END AS data,
                summary, user_verified
                FROM attachments WHERE project_id = ${projectId} ORDER BY id`;
  return rows;
}
